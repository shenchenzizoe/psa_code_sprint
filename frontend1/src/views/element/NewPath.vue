...
export default {
    ...
    const requestData = {
      ...
      distance: parseFloat(this.formLabelAlign.distance), // Convert distance to a number
    };

    axios.post('http://localhost:3333/paths/add', requestData, {
      headers: {
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          this.$alert('Failed to create path.', 'Error', {
            confirmButtonText: 'OK',
            callback: action => {
              this.$message({
                type: 'error',
